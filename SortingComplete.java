public class SortingComplete{
    ...
    public static void  quickSort3(int[] x, int low, int high)
    { // The essence of quicksort3 is that there are 3 partitions instead of 2
        // here we get the lowest index, greatest index and the array as arguments for the function
        if (high <= low)
            return;
        ...
        int element = x[low];
        int i = low;
// we have chosen i as low index , this changes as we are using recursion
        //while loop chosen to perform swaps as needed
        while (i <= grind) {
            if (x[i] < element)
                swap(x, loind++, i++);
            else if (x[i] > element)
...
// recursion for the partitions
        quickSort3(x, low, loind - 1);
        quickSort3(x, grind + 1, high);
    }
